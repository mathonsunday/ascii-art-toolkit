 ...
 */
export function validatePiece(piece: AsciiArt): { valid: boolean; errors: string[] } {
  const errors: string[] = [];

  // Required string fields
  if (!piece.id) errors.push('Missing id');
  if (!piece.name) errors.push('Missing name');
  ...
  if (!piece.description) errors.push(`Piece ${piece.id}: missing description`);
  if (!piece.whyEffective) errors.push(`Piece ${piece.id}: missing whyEffective`);
  ...
  if (piece.id && piece.theme && !piece.id.startsWith(`${piece.theme}:`)) {
    errors.push(
      `Piece ${piece.id}: ID should be prefixed with "${piece.theme}:" to match theme`
    );
  }
 ...
 */
export function validateRendering(art: string): { valid: boolean; warnings: string[] } {
 ...
 */
export function validateThemePieces(theme: string): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} {
 ...
 */
export function validateZoomVariants(
  piece: AsciiArt
  ...
  ['far', 'medium', 'close'].forEach((level) => {
    const art = piece.zoom![level as keyof typeof piece.zoom];
    if (art) {
