...
export class AsciiLibrary {
  ...
  private allPiecesById: Map<string, AsciiArt> = new Map();
  ...
  addPiece(theme: string, piece: AsciiArt): void {
    ...
    if (!piece.id.startsWith(`${theme}:`)) {
      throw new Error(
        `Piece ID "${piece.id}" should start with "${theme}:" for theme "${theme}"`
      );
    }
  ...
  getTagMetadata(theme: string): TagMetadata {
    ...
    pieces.forEach((piece) => {
      Object.entries(piece.tags).forEach(([key, value]) => {
        if (!metadata[key]) {
          let type: 'string' | 'boolean' | 'string[]' | 'enum' = 'string';
          if (Array.isArray(value)) {
            type = 'string[]';
        ...
        if (Array.isArray(value)) {
          value.forEach((v) => {
            if (
              !metadata[key].values!.includes(v) &&
          ...
          if (
            !metadata[key].values!.includes(value) &&
  ...
  validate(): ValidationResult {
    ...
    this.allPiecesById.forEach((piece) => {
      if (!piece.id) errors.push(`Piece missing id`);
      if (!piece.name) errors.push(`Piece ${piece.id} missing name`);
      if (!piece.category) errors.push(`Piece ${piece.id} missing category`);
      if (!piece.theme) errors.push(`Piece ${piece.id} missing theme`);
      if (!piece.art) errors.push(`Piece ${piece.id} missing art`);
      if (!piece.description) warnings.push(`Piece ${piece.id} missing description`);
      if (!piece.whyEffective) warnings.push(`Piece ${piece.id} missing whyEffective`);
  ...
  getStats(): {
    totalPieces: number;
    themes: Record<string, { pieces: number; categories: string[] }>;
  } {
    const stats: Record<string, { pieces: number; categories: string[] }> = {};
